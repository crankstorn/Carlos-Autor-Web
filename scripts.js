// Se envuelve todo el código en un único listener para asegurar que se ejecuta
// solo cuando el HTML de la página está completamente cargado.
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ÚLTIMAS NOTICIAS EN LA PÁGINA DE INICIO ---
    ...
    if (newsContainer) {
        ...
        const loadLatestNews = async () => {
            try {
                ...
                const latestPosts = (await response.json()).slice(0, 5);
                ...
                latestPosts.forEach(post => {
                    ...
                    newsItem.className = 'flex items-center gap-6 border-b border-gray-200 pb-6';
                    newsItem.innerHTML = `
                        <div class="w-1/3 md:w-1/4 text-sm text-zinc-500 uppercase tracking-wider font-sans">${formatNewsDate(date)}</div>
                        <div class="w-2/3 md:w-3/4 text-xl"><a href="/blog/${slug}" class="text-zinc-800 hover:text-[--color-accent] transition-colors">${title}</a></div>
                    `;
